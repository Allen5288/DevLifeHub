...
const CurrencyExchange = () => {
  ...
  const generateHistoricalData = useCallback((baseRate, timeRange) => {
  ...
  const fetchRates = useCallback(async () => {
    ...
    setShowRates(true);
    try {
      const apiKey = process.env.REACT_APP_CURRENCY_EXHAUSTION_API_KEY;
      ...
      const response = await fetch(
        `https://v6.exchangerate-api.com/v6/${apiKey}/latest/${baseCurrency}?_=${timestamp}`
      );
  ...
  }, [baseCurrency, timeRange, generateHistoricalData]);
  ...
  }, [autoRefresh, showRates, fetchRates]);
